...
const FormBuilder = function(opts, element) {
  ...
  const defaultFieldAttrs = type => {
    ...
    const typeAttrsMap = {
      ...
      urlautocomplete: ['required','label','description','name','url','otherlabel'],

      autocomplete: defaultAttrs.concat(['options']),
      button: ['label', 'subtype', 'style', 'className', 'name', 'value', 'access','url','inlinebtn'],
      checkbox: [
        'required',
        'label',
        'description',
      ...
      date: defaultAttrs.concat(['errmsg']),
        tel: defaultAttrs.concat(['errmsg']),
        email: defaultAttrs.concat(['errmsg']),
      ...
      number: defaultAttrs.concat(['errmsg','min', 'max', 'step']),
  ...
  let advFields = values => {
    ...
    const advFieldMap = {
      ...
      inline: () => {
        ...
        inlinebtn: ()=> boolAttribute('inlinebtn', values, { first: i18n.inline }),
        label: () => textAttribute('label', values),
        url: () => textAttribute('url', values),
        errmsg: () => textAttribute('errmsg', values),
        otherlabel: () => textAttribute('otherlabel', values),
        payload:() => textAttribute('payload', values),
        action: () => textAttribute('action', values),
        configJson: () => textAttribute('configJson', values),
  ...
  let selectFieldOptions = function(name, optionData, multipleSelect) {
    ...
    let optionDataOrder = ['configJson','action', 'value', 'label', 'selected']
    let optionInputs = []
    let optionTemplate = { selected: false, label: '', value: '', action:'',configJson:'' }
    ...
    for (let i = optionDataOrder.length - 1; i >= 0; i--) {
      ...
      if (optionData.hasOwnProperty(prop)) {
        ...
        if(prop=='configJson'){
            ...
            attrs.value=JSON.stringify(attrs.value)
            if(attrs.value=='""'){
                attrs.value='';
            }
        }
